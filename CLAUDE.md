...
**성능 최적화 원칙**
- **측정 전 최적화 금지**: 실제 성능 문제가 확인된 후에만 최적화
- **단순함 우선**: 복잡한 최적화보다 단순하고 정확한 코드
- **React.memo 신중히 사용**:
  - 함수 props가 있으면 사용 지양 (stale closure, 복잡한 비교 로직)
  - 비용이 큰 컴포넌트에만 적용
  - 커스텀 비교 함수는 모든 props를 정확히 처리해야 함
- **useMemo/useCallback**:
  - 비용이 큰 계산만 메모이제이션
  - 함수는 안정적 참조가 필요할 때만 useCallback
  - 의존성 배열 정확히 관리
- **GPU 가속 활용**: transform, opacity, will-change
...
### Props가 전달되지 않는 문제
- **React.memo의 커스텀 비교 함수 확인**:
  - 함수 props를 비교하지 않으면 컴포넌트가 리렌더링되지 않음
  - 이전 props(undefined)를 계속 사용하는 버그 발생 가능
  - 해결: memo 제거 또는 모든 props를 정확히 비교
- **Stale Closure 확인**:
  - useCallback/useMemo 의존성 배열 누락
  - ref를 사용해 최신 값 유지 고려
- **Props 참조 안정성**:
  - 매번 새로운 객체/함수/배열을 생성하지 않는지 확인
  - 필요시 useMemo/useCallback 사용
...
**사례: React.memo 과용으로 인한 Lightbox 버그**

**문제 발생**:
- `CarouselItem`에 커스텀 비교 함수를 가진 `React.memo` 적용
- 함수 props(`onLightboxOpen`, `onItemClick`)를 비교하지 않음
- 컴포넌트가 리렌더링되지 않아 이전 props(undefined) 계속 사용
- 결과: Lightbox가 전혀 작동하지 않음

**교훈**:
1. **측정 없는 최적화는 하지 말 것**
   - 실제 성능 문제가 없으면 단순하게 유지
   - 프로파일링으로 병목 지점 확인 후 최적화

2. **React.memo는 신중하게**
   - 함수 props가 있으면 사용 지양
   - 커스텀 비교 함수는 모든 props를 정확히 처리해야 함
   - 빠뜨린 props로 인한 버그는 디버깅이 어려움

3. **단순함이 최고의 최적화**
   - memo 제거 후 오히려 번들 크기 감소 (~700 bytes)
   - 코드가 단순해지고 버그 해결
   - 아이템 수(최대 30개)와 렌더링 비용 고려 시 memo 불필요

4. **함수 참조 안정화의 복잡성**
   - 모든 함수 props에 useCallback 필요
   - 의존성 배열 관리 복잡도 증가
   - 실익 대비 복잡도가 너무 높음

**결론**:
- 단순하고 정확한 코드 > 복잡하고 빠른 코드
- 필요할 때까지 최적화 미루기
- 최적화는 측정 가능한 문제에만 적용
