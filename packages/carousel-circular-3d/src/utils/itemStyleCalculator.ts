...
export interface ItemStyleParams {
  ...
  /** 크기 스케일 범위 [최소, 최대] - transform scale 효과 고려 */
 ...
 * scaleRange, perspective, translateZ를 모두 고려하여 실제 렌더링 크기가 containerHeight를 초과하지 않도록 한다.
 * @param params - 스타일 계산 파라미터
 * @returns CSS 스타일 객체
 */
export function calculateItemStyle(params: ItemStyleParams): CSSProperties {
  const { containerHeight, orientation, transform, scaleRange, perspective, radius, isClickable } =
    params;
  ...
  // 4. 전체 확대 배율 = scale × perspectiveScale
